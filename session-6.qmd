...
La moyenne arithmétique, souvent notée $\bar{x}$, est l'indicateur de tendance centrale le plus connu. Elle est définie, en général, comme étant le quotient de la somme de toutes les valeurs d'une série par le nombre d'observations.
...
```{r}
# Afficher, sous forme de tableau, la variable mpg du jeu de données mtcars
knitr::kable(t(mtcars[,c("mpg")]),
             caption = "Les miles par gallons des voitures du jeu de données mtcars")
```
...
knitr::kable(t(mtcars[,c("cyl")]),
             caption = "Les cylindrées des voitures du jeu de données mtcars")
```
